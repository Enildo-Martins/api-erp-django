...
class GroupsPermission(permissions.BasePermission):
    message = 'O funcionário não tem permissao de gerenciar os grupos'

    def has_permission(self, request, _view):
        return check_permission(request.user, request.method, permission_to='group')
    
class GroupsPermissionsPermissions(permissions.BasePermission):
    message = 'O funcionário não tem permissao para gerenciar as permissões'
    
    def has_permission(self, request, _view):
        return check_permission(request.user, request.method, permission_to='permission') 
    
class TaskPermission(permissions.BasePermission):
    message = 'O funcionário nao tem permissão para gerenciar as tarefas de todos os funcionários'
